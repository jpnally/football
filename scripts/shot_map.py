...
shots['colour'] = shots['shot_outcome'].apply(lambda outcome: 'green' if outcome == 'Goal' else 'red')

pitch = Pitch(pitch_type='statsbomb', pitch_color='grass', line_color='white', stripe=True)
fig, ax = pitch.draw(figsize = (18,12))

ax.scatter(shots['x'], shots['y'], color = shots['colour'], s=100, alpha = 0.75, zorder = 3)
